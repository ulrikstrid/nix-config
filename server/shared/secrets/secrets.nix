...
in
{
  ...
  "hydra-signing-key.age".publicKeys = users ++ [
    servern
    anmonteiro
  ];
  "hydra_aws_credentials.age".publicKeys = users ++ [ servern ];
  # Stridbot
  "stridbot_signing_key.age".publicKeys = users;
  "stridbot_env.age".publicKeys = users;
  # Home assistant
  ...
  "mosquitto-zigbee2mqtt.age".publicKeys = users ++ [
    servern
    nuc-01
  ];
  # Trafeik acme
  "traefik-env.age".publicKeys = users ++ [
    servern
    odroid-n2-01
  ];
